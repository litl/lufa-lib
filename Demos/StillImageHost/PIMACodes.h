/*
             MyUSB Library
     Copyright (C) Dean Camera, 2008.
              
  dean [at] fourwalledcubicle [dot] com
      www.fourwalledcubicle.com

 Released under the LGPL Licence, Version 3
*/

#ifndef _PIMA_CODES_H_

	/* Macros: */
		#define PIMA_OPERATION_GETDEVICEINFO    0x1001
		#define PIMA_OPERATION_OPENSESSION      0x1002
		#define PIMA_OPERATION_CLOSESESSION     0x1003
		#define PIMA_OPERATION_GETNUMOBJECTS    0x1006
		#define PIMA_OPERATION_POWERDOWN        0x1013
		#define PIMA_OPERATION_INITIATECAPTURE  0x100E
		#define PIMA_OPERATION_GETSTORAGEIDS    0x1004
		
		#define PIMA_RESPONSE_OK                0x2001

#endif
